  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: '已學會', value: knownCount, color: 'text-emerald-600' },
          { label: '收藏', value: favCount, color: 'text-amber-500' },
          { label: '錯題', value: wrongCount, color: 'text-rose-500' },
        ].map((s) => (
          <div
            key={s.label}
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <button
          ...
          className="bg-blue-600 text-white rounded-xl px-4 py-5 hover:bg-blue-700 text-left"
        >
        ...
        <button
          ...
          className="bg-white border border-slate-200 rounded-xl px-4 py-5 hover:bg-slate-50 text-left"
        >
        ...
        >
          ...
          <div className="text-sm text-slate-500 mt-1">查詢、收藏、標記</div>
        </button>
      </div>
          ...
          <ul className="divide-y divide-slate-100 text-sm">
            {recent.map((r, i) => (
              <li key={i} className="px-4 py-3 flex items-center gap-3">
                <span className="text-slate-400 w-20">{formatDate(r.date)}</span>
      ...
      {(knownCount > 0 || favCount > 0 || wrongCount > 0 || recent.length > 0) && (
        <div className="text-center">
          <button
            ...
            className="text-sm text-slate-400 hover:text-rose-600 underline underline-offset-2"
          >
            清除所有學習進度
          </button>
        </div>
